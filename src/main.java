...
class ChessFrame extends JFrame implements ActionListener {
    private String[] strsize={"15x15","30x30","40x30"};
    private String[] strmode={"人机对弈(人先手)","人机对弈(机器先手)"};
    ...
    public JMenu makeMenu(Object parent, Object items[], Object target){
        ...
        for(int i = 0; i < items.length; i++)
            if(items[i] == null)
                m.addSeparator();
            else if(items[i] == "人人对弈"){
                JMenu jm = new JMenu("人人对弈");
                ...
                m.add(jm);
            }else if(items[i] == "人机对弈"){
                JMenu jm = new JMenu("人机对弈");
                ButtonGroup group=new ButtonGroup();
                JRadioButtonMenuItem rmenu;
...
//-5代表该棋盘方格不能够下棋子
    ...
    public ChessModel(int modeChess){
        this.isOdd=true;
        if(modeChess == 1){
            PanelInit(15, 15, modeChess);
        }
        else if(modeChess == 2){
            PanelInit(30, 30, modeChess);
        }
        else if(modeChess == 3){
            PanelInit(45, 45, modeChess);
        }
        else if(modeChess == 4) {
            PanelInit(30, 30, modeChess);
        }
...
//根据下棋判断胜负等
    ...
    public void competerPressed(MouseEvent evt){
        int x = (evt.getX()-10) / 20;
        int y = (evt.getY()-10) / 20;
        System.out.println(x+" "+y);
        if (evt.getModifiers()==MouseEvent.BUTTON1_MASK){
